...
export default class Resizable extends React.Component<Props, State> {
  ...
  refResizableHandle: ?HTMLSpanElement;

  listenerScroll(node: HTMLElement): void {
    if (this.refResizableHandle) {
      this.refResizableHandle.style.bottom = -1 * node.scrollTop + 'px';
    }
  }

  componentDidMount() {
    const node = this.refChild;

    if (node) {
      node.addEventListener('scroll', () => this.listenerScroll(node));
    }
  }
  ...
  componentWillUnmount() {
    ...
    if (node) {
      node.removeEventListener('scroll', () => this.listenerScroll(node));
    }
  }
  ...
  resizeHandler(handlerName: string, axis: ResizeHandle): Function {
    return (e: SyntheticEvent<> | MouseEvent, {node, deltaX, deltaY}: DragCallbackData) => {
      ...
      if (handlerName === 'onResizeStart') {
        ...
        // reverse position resizable handel (icon)
        if (this.refResizableHandle) {
          this.refResizableHandle.style.bottom = '0';
        }
      } else {
  ...
  renderResizeHandle(resizeHandle: ResizeHandle): ReactNode {
    ...
    return <span
        ...
        ref={(obj) => this.refResizableHandle = obj}
    />;
  }
